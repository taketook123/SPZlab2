...
/* 
Наступний кеш cache_Userland_slab використовується внутрішньо для виділення пам'яті
об'єктам Userland_slab щоразу, коли потрібно створити новий кеш для
об'єктів, яким не потрібно, щоб структури Userland_slab містилися в самому новому кеші.

Для того, щоб уникнути рекурсії, яка виникає у випадку, коли потрібно виділити новий кеш
для кешу cache_Userland_slab, його кеші міститимуть їхні дескриптори (SLAB_DESCR_ON_SLAB).
*/
...
/* Створює новий блок для додавання до кешу.

    pgs_per_slab: кількість сторінок, які використовує цей блок
    pg_sz: розмір сторінки в байтах
    obj_sz: (фактичний) розмір об'єкта
    cache_slab_descr: якщо не null, то кеш, з якого виділяється дескриптор блоку замість того, щоб зберігати його на початку блоку.
    Повертає адресу дескриптора нового блоку у разі успіху, NULL у разі помилки
 */
					 ...
					 struct Objs_cache *cache_slab_descr)
{
  ...
  void *new_slab_pgs = mmap(NULL,
			    ...
			    -1,
			    0);
  //Примітка: немає необхідності очищувати сторінки, оскільки прапор MAP_ANONYMOUS означає, що вони вже ініціалізовані нулями.
  ...
  //На початку кожної сторінки ми визначаємо вказівник на дескриптор блоку, до якого належить ця сторінка.
  ...
  //ми налаштовуємо зв'язаний список вільних об'єктів
  ...
  if (cache != NULL) {
    ...
    if (!dlist_is_empty_generic(cache->partial_slabs)) {
      ...
      if (dlist_is_empty_generic(cache->free_slabs)) {
        ...
        /* Оновлюємо лічильники блоків та об'єктів */
      ...
      /* Отримуємо вказівник на вільний блок */
      struct Userland_slab *slab = cache->free_slabs;

      /* Виділяємо об'єкт з блоку */
      allocated_obj = alloc_obj_from_slab(slab);

      /* Перевірка на помилку виділення */
      if (allocated_obj == NULL) {
        printf("Не вдалося виділити об'єкт у %s (пошкоджений блок)!\n", __func__);
        return NULL;
      }
      ...
      /* Якщо блок частково використаний */
      if (!is_slab_full(slab)) {
        /* Переміщуємо блок з вільних у частково використані */
        dlist_delete_head_generic(cache->partial_slabs, slab, prev, next);
        ...
        /* Особливий випадок: блок може містити лише один об'єкт */
